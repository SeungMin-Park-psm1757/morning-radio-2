...
class TQICollector(BaseCollector):
    ...
    def _parse_html_list(self, html: str, config: SourceConfig) -> list:
        ...
        items = []
        for article in soup.select("article.elementor-post")[: config.max_items_per_run]:
            link = article.select_one(".elementor-post__title a")
            date_node = article.select_one(".elementor-post-date")
            author_node = article.select_one(".elementor-post-author")
            if link is None:
                continue
            items.append(
