...
def score_article(item: NormalizedArticle, *, now: datetime) -> float:
    ...
    if item.raw.published_at:
        ...
        score += max(0.0, 96.0 - min(hours, 96.0)) / 12.0

    score += SOURCE_WEIGHTS.get(item.raw.source_key, 1.0)
...
def score_cluster(cluster: StoryCluster) -> float:
    ...
    cluster.cluster_score = representative_score + multi_source_bonus + size_bonus
