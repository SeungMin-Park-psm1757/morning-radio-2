...
class QuantumFrontiersCollector(BaseCollector):
    ...
    def _parse_html_list(self, html: str, config: SourceConfig) -> list:
        ...
        items = []
        for article in soup.select("article.post")[: config.max_items_per_run]:
            link = article.select_one("h1 a, h2 a, h3 a")
            time_node = article.select_one("time.entry-date")
            if link is None:
                continue
            items.append(
                self.build_raw_article(
                    ...
                    published_at=self.parse_datetime(time_node.get("datetime") if time_node else None),
